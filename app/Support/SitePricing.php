...
{
    /**
     * Returns the turnaround code actually used for billing, which may differ
     * from the one the customer selected based on their subscription plan.
     * Studio/Production: Priority→Standard, SuperRush→Priority, FlashRush→SuperRush.
     * Enterprise: SuperRush→Standard, FlashRush→SuperRush.
     */
    public static function effectivePricingTurnaround(string $turnaroundCode, ?AdminUser $customer): string
    {
        $plan = strtolower(trim((string) ($customer?->subscription_plan ?? '')));

        if (in_array($plan, ['production', 'enterprise'], true)) {
            return match ($turnaroundCode) {
                'priority'  => 'standard',
                'superrush' => 'priority',
                'flashrush' => 'superrush',
                default     => $turnaroundCode,
            };
        }
    ...
    public static function embroidery(
        ...
        $billingCode    = self::effectivePricingTurnaround($turnaroundCode, $customer);
        $site = self::siteContext($website, $siteId);
        $profile = self::profileFor($site, ['digitizing', 'embroidery'], $billingCode);
        $error = self::configurationError($customer, $website, $siteId, 'digitizing', $billingCode);

        if ($error !== null) {
            throw new \RuntimeException($error);
        }

        $rate = self::customerRateOverride($customer, $billingCode);
        $hasCustomerOverride = $rate !== null;
        if ($rate === null) {
            $rate = self::embroideryRateFromProfile($profile, $billingCode);
        }

        $minimum = self::minimumCharge($profile, $billingCode, $rate, $hasCustomerOverride);
    ...
    public static function normalizeTurnaround(?string $turnaroundTime): string
    {
        return match (strtolower(trim((string) $turnaroundTime))) {
            ...
            'flashrush', 'flash_rush', 'flash-rush', 'flash rush' => 'flashrush',
            default => 'standard',
        };
    }
    ...
    public static function turnaroundFeeSchedule(?AdminUser $customer, ?SiteContext $site, string $workType, bool $useCustomerOverrides = true): array
    {
        ...
        foreach (['standard', 'priority', 'superrush', 'flashrush'] as $turnaroundCode) {
            // Resolve which rate tier is actually billed for this subscription plan.
            $billingCode = self::effectivePricingTurnaround($turnaroundCode, $customer);

            if (in_array($normalizedWorkType, ['vector', 'color'], true)) {
                $profile = self::profileFor($site, [$normalizedWorkType], $billingCode);
            ...
            $profile = self::profileFor($site, ['digitizing', 'embroidery'], $billingCode);
            $customerRate = $useCustomerOverrides ? self::customerRateOverride($customer, $billingCode) : null;
            $rate = $customerRate ?? self::embroideryRateFromProfile($profile, $billingCode);
            $minimum = self::minimumCharge($profile, $billingCode, $rate ?? 0.0, $customerRate !== null);
