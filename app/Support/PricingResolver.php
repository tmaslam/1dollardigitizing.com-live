...
{
    public static function firstStitchUnit(string $units): string
    {
        $units = trim($units);
    ...
    public static function forAdminEntry(
        ...
        $firstUnit = self::firstStitchUnit($units);
        if ($firstUnit === '' || preg_match('/^\d+(\.\d+)?$/', $firstUnit) !== 1 || (float) $firstUnit <= 0) {
            return [
                'ok' => false,
                'message' => 'Enter a numeric stitch count to calculate the price. Use / to separate multiple files (e.g. 12000/8000).',
            ];
        }
