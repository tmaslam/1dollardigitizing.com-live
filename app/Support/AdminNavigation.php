...
class AdminNavigation
{
    public static function counts(): array
    {
        return [
            ...
            'new_quotes' => Order::query()->active()->quoteManagement()->where('status', 'Underprocess')->unassigned()->where(fn ($q) => $q->whereNull('assigned_group')->orWhere('assigned_group', ''))->count(),
            'assigned_quotes' => Order::query()->active()->quoteManagement()->where('status', 'Underprocess')
                ->where(function ($q) {
                    $q->assigned()
                      ->orWhere(function ($q2) {
                          $q2->whereNotNull('assigned_group')->where('assigned_group', '!=', '');
