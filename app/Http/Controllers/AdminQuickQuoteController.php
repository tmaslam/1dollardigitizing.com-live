...
class AdminQuickQuoteController extends Controller
{
    ...
    public function downloadAttachment(Attachment $attachment)
    {
        ...
        if (! is_file($path)) {
            SecurityAudit::record(null, 'files.attachment_missing', 'Admin quick-quote attachment download failed because the file was not found on disk.', [
    ...
    public function previewAttachment(Request $request, Attachment $attachment)
    {
        ...
        if (! is_file($path)) {
            SecurityAudit::record(null, 'files.attachment_preview_missing', 'Admin quick-quote attachment preview failed because the file was not found on disk.', [
                'attachment_id' => (int) $attachment->id,
                'order_id' => (int) $attachment->order_id,
                'file_source' => (string) $attachment->file_source,
                'resolved_path' => $path,
    ...
    public function complete(Request $request)
    {
        ...
        if (! preg_match('/^\d+(\.\d+)?(\/\d+(\.\d+)?)*$/', $stitches)) {
            return redirect()->to(url('/v/view-quick-order-detail.php?oid='.$order->order_id.'&page=qquote'))
    ...
    private function attachmentAbsolutePath(Attachment $attachment): string
    {
        $folder = match ($attachment->file_source) {
            'qquote', 'edit order', 'orderTeamImages' => 'quotes',
            'scanned' => 'scanned',
            'team' => 'team',
            'sewout' => 'sewout',
            default => 'order',
