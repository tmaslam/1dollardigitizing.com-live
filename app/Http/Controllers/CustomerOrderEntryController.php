...
class CustomerOrderEntryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (! $placement['can_place']) {
            return back()->withInput()->withErrors(['placement' => $placement['warning']]);
        }
        ...
        $isQuoteSubmission = in_array($orderType, ['digitzing', 'q-vector'], true);
        ...
        $order = Order::query()->create([
            ...
            'del_attachment' => 0,
            'type' => $type,
            'assigned_group' => $isQuoteSubmission ? 'inhouse' : null,
        ]);
    ...
    {
        $customer = $this->customer($request);
        $allowedOptions = $this->turnaroundOptionsForCustomer($customer);
        $commentsRule = ['nullable', 'string', 'max:5000'];
        $sourceFilesRule = ['nullable', 'array'];
        ...
        return $request->validate([
            ...
            'turn_around_time' => ['required', 'in:'.implode(',', $allowedOptions)],
    ...
    private function turnaroundOptionLabels(AdminUser $customer, SiteContext $site, string $workType): array
    {
        ...
        foreach ($this->turnaroundOptionsForCustomer($customer) as $option) {
            $code        = SitePricing::normalizeTurnaround($option);
            $billingCode = SitePricing::effectivePricingTurnaround($code, $customer);
            $timingLabel = \App\Support\TurnaroundTracking::labelWithTiming($option);
            $description = $schedule[$code]['description'] ?? null;
            $planBenefit = ($billingCode !== $code)
                ? ' ★ plan benefit — billed at '.($tierLabels[$billingCode] ?? $billingCode).' rate'
                : '';

            $labels[$option] = $description
                ? $timingLabel.' - '.$description.$planBenefit
                : $timingLabel.$planBenefit;
        }

        return $labels;
    }

    private function turnaroundOptionsForCustomer(AdminUser $customer): array
    {
        ...
        return match ($plan) {
            ...
            default                      => self::TURNAROUND_OPTIONS,
        };
    }
    ...
    {
        return Order::nextCustomerOrderNumber($customer->user_id, $site->legacyKey);
    }
