...
class AdminToolsController extends Controller
{
    ...
    public function dueReportDetail(Request $request)
    {
        ...
        $currentBalance = CustomerBalance::deposit(AdminUser::query()->where('user_id', $userId)->value('topup'));
    ...
    public function teamReport(Request $request)
    {
        ...
        $teams = AdminUser::query()
            ...
            ->active()
            ->where('is_active', 1)
            ->when($groupFilter === 'inhouse', fn ($q) => $q->where('team_group', 'inhouse'))
            ->when($groupFilter === 'freelance', fn ($q) => $q->where('team_group', 'freelance'))
        ...
        $baseQuery = Order::query()
            ->active()
            ->with('assignee:user_id,user_name,team_group')
            ->where('status', 'approved')
            ->when($request->filled('team'), fn (Builder $q) => $q->where('assign_to', (int) $request->query('team')))
            ->when($request->filled('month'), fn (Builder $q) => $q->where('completion_date', 'like', trim((string) $request->string('month')).'%'))
            ->when(! $request->filled('team') && ! $request->filled('month'), fn (Builder $q) => $q->where('completion_date', 'like', now()->format('Y').'%'))
            ->when($groupFilter !== '', fn (Builder $q) => $q->whereHas('assignee', fn (Builder $u) => $u->where('team_group', $groupFilter)));

        $orders = $baseQuery->orderBy(
        ...
        $quotesByOrder = FreelanceQuote::query()
            ...
            ->whereIn('order_id', $orderIds)
            ->get(['order_id', 'team_user_id', 'quoted_price'])
            ->keyBy('order_id');

        $finesByOrder = TeamFine::query()
            ->whereIn('order_id', $orderIds)
            ->get(['order_id', 'team_user_id', 'amount', 'reason'])
            ->keyBy('order_id');

        $orders = $orders->map(function (Order $order) use ($reviewedLookup, $quotesByOrder, $finesByOrder) {
            $order->setAttribute('supervisor_checked_flag', $reviewedLookup->has((int) $order->order_id));
            $isFreelanceOrder = ($order->assignee?->team_group ?? '') === 'freelance';
            $order->setAttribute('is_freelance_order', $isFreelanceOrder);

            if ($isFreelanceOrder) {
                $quote = $quotesByOrder[(int) $order->order_id] ?? null;
                $fine  = $finesByOrder[(int) $order->order_id] ?? null;
                $gross = (float) ($quote?->quoted_price ?? 0);
                $fineAmt = (float) ($fine?->amount ?? 0);
                $order->setAttribute('freelance_gross_pkr', $gross);
                $order->setAttribute('freelance_fine_pkr', $fineAmt);
                $order->setAttribute('freelance_net_pkr', max(0, $gross - $fineAmt));
            } else {
                $fine = $finesByOrder[(int) $order->order_id] ?? null;
                $order->setAttribute('inhouse_fine_rs', (float) ($fine?->amount ?? 0));
            }
        ...
        if ($request->query('export') === 'csv') {
            ...
            $callback = function () use ($orders) {
                ...
                fputcsv($handle, ['Order ID', 'Design Type', 'Design Name', 'Assigned To', 'Group', 'Supervisor Checked', 'Stitches', 'Total Amount', 'Completion Date', 'Accepted Price (PKR)', 'Fine (PKR/Rs.)', 'Net (PKR)']);

                foreach ($orders as $order) {
                    $isFreelance = (bool) $order->is_freelance_order;
                    fputcsv($handle, [
                        $order->order_id,
                        $order->work_type_label,
                        $order->design_name ?: '',
                        $order->assignee?->user_name ?: '',
                        $isFreelance ? 'Freelance' : 'In-House',
                        ...
                        $isFreelance ? number_format((float) $order->freelance_gross_pkr, 2, '.', '') : '',
                        $isFreelance
                            ? ((float) $order->freelance_fine_pkr > 0 ? number_format((float) $order->freelance_fine_pkr, 2, '.', '') : '')
                            : ((float) $order->inhouse_fine_rs > 0 ? number_format((float) $order->inhouse_fine_rs, 2, '.', '') : ''),
                        $isFreelance ? number_format((float) $order->freelance_net_pkr, 2, '.', '') : '',
                    ]);
                }
        ...
        return view('admin.tools.team-report', [
            ...
            'summary'      => [
                ...
                'total_net_pkr'      => $orders->filter(fn ($o) => (bool) $o->is_freelance_order)->sum(fn ($o) => (float) $o->freelance_net_pkr),
            ],
        ]);
    }
