...
class TeamOrderDetailController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        // Freelance quote state for this user on this order
        $myFreelanceQuote = null;
        ...
        if ($isFreelancePool && $teamUser->isFreelance()) {
            $myFreelanceQuote = FreelanceQuote::query()
                ->where('order_id', $order->order_id)
                ->where('team_user_id', $teamUser->user_id)
                ->first();
        }
        if ($isFreelancePool && ($teamUser->is_supervisor ?? false)) {
        ...
        return view('team.orders.show', [
            ...
            'customerComments' => OrderComment::query()->where('order_id', $order->order_id)->where('comment_source', 'customerComments')->where('comments', '!=', '')->latest('id')->get(),
            ...
            'myFreelanceQuote' => $myFreelanceQuote,
    ...
    public function downloadAttachment(Request $request, Attachment $attachment)
    {
        ...
        if (! is_file($path)) {
            SecurityAudit::record(null, 'files.attachment_missing', 'Team attachment download failed because the file was not found on disk.', [
                'attachment_id' => (int) $attachment->id,
                'order_id' => (int) $attachment->order_id,
                'file_source' => (string) $attachment->file_source,
                'resolved_path' => $path,
                'file_name_with_date' => (string) $attachment->file_name_with_date,
            ], 'warning');
    ...
    public function previewAttachment(Request $request, Attachment $attachment)
    {
        ...
        if (! is_file($path)) {
            SecurityAudit::record(null, 'files.attachment_preview_missing', 'Team attachment preview failed because the file was not found on disk.', [
                'attachment_id' => (int) $attachment->id,
                'order_id' => (int) $attachment->order_id,
                'file_source' => (string) $attachment->file_source,
                'resolved_path' => $path,
    ...
    private function attachmentAbsolutePath(Attachment $attachment, Order $order, string $mode): string
    {
        ...
        $folder = match ($source) {
            'color', 'edit order' => $mode === 'quote' ? 'quotes' : 'order',
            'orderTeamImages' => 'team',
            'team' => 'team',
            'sewout' => 'sewout',
            'scanned' => 'scanned',
            'quote', 'qquote' => 'quotes',
        ...
        $fallbackFolders = match (true) {
            in_array($source, ['order', 'edit order', 'vector', 'color'], true) => ['quotes'],
            $source === 'orderTeamImages' => ['order', 'quotes'],
            $source === 'team' => ['sewout'],
            default => [],
        };
    ...
    private function notifySupervisors(string $teamMemberName, Order $order): void
    {
        $supervisors = AdminUser::supervisors()->active()->whereNotNull('user_email')->where('user_email', '!=', '')->get();

        if ($supervisors->isEmpty()) {
            return;
        }

        $subject = 'Job ready for review — Order #'.$order->order_id;

        foreach ($supervisors as $supervisor) {
            $body = view('team.emails.supervisor-job-complete', [
                ...
                'reviewUrl'       => url('/team/supervisor/review-queue'),
            ])->render();

            PortalMailer::sendHtml($supervisor->user_email, $subject, $body);
        }
    }
}
