...
class CustomerPaymentController extends Controller
{
    ...
    public function paySingleWithCredit(Request $request, Billing $billing)
    {
        $customer = $this->customer($request);
        $site = $this->site($request);
        CustomerBalance::settleZeroAmountBillings((int) $customer->user_id, $site->legacyKey, 'system-auto');
        ...
        if (! $billing) {
            return redirect(url('/view-billing.php'))->with('success', 'This invoice is already settled.');
        }

        $paid = CustomerBalance::applyToBilling($billing, 'credit-payment');

        if (! $paid) {
            $amount    = $this->billingAmount($billing);
            $available = CustomerBalance::deposit($customer->topup);

            return redirect(url('/view-billing.php'))->withErrors([
                'credit' => 'Insufficient credit balance. You have '.number_format($available, 2).' credits but this invoice requires '.number_format($amount, 2).' credits. Top up your balance to pay this invoice.',
            ]);
        }

        return redirect(url('/view-billing.php'))->with('success', 'Invoice INV-'.$billing->bill_id.' has been paid from your credit balance and the order has been moved to paid orders.');
    }

    public function payAllWithCredit(Request $request)
    {
        $customer = $this->customer($request);
        $site = $this->site($request);
        CustomerBalance::settleZeroAmountBillings((int) $customer->user_id, $site->legacyKey, 'system-auto');
        ...
        if ($billings->isEmpty()) {
            return redirect(url('/view-billing.php'))->with('success', 'No unpaid invoices remain.');
        }

        $totalAmount = round($billings->sum(fn (Billing $b) => $this->billingAmount($b)), 2);
        $available = CustomerBalance::deposit($customer->topup);

        if ($available < $totalAmount - 0.001) {
            return redirect(url('/view-billing.php'))->withErrors([
                'credit' => 'Insufficient credit balance to pay all invoices. You have '.number_format($available, 2).' credits but all invoices total '.number_format($totalAmount, 2).' credits. Please top up your balance or pay individual invoices instead.',
            ]);
        }

        $paid = 0;
        foreach ($billings as $billing) {
            if (CustomerBalance::applyToBilling($billing, 'credit-payment')) {
                $paid++;
            }
        }

        return redirect(url('/view-billing.php'))->with('success', $paid.' invoice'.($paid === 1 ? '' : 's').' totalling '.number_format($totalAmount, 2).' credits paid from your balance. Orders have been moved to paid orders.');
    }
    ...
    public function handleStripeWebhook(Request $request)
    {
        ...
        if ($transaction && (string) $transaction->payment_scope === 'credit_purchase'
            && in_array($type, ['checkout.session.completed', 'checkout.session.async_payment_succeeded'], true)
            && $paymentStatus === 'paid'
            && (string) $transaction->status !== 'verified'
        ) {
            $confirmedAmount = StripeHostedCheckout::confirmedAmount($session);
            $providerRef     = StripeHostedCheckout::providerReference($session);

            // Credit the full_price (credit packs) or plan_credits (subscriptions), not the discounted paid amount.
            ...
            $creditAmount = $fullPrice > 0
                ? $fullPrice
                : ($planCredits > 0 ? (float) $planCredits : ($confirmedAmount > 0 ? $confirmedAmount : (float) $transaction->requested_amount));
            ...
            $planUser = null;
            if ($isSignup) {
                $planUser = \App\Models\AdminUser::query()
                    ->customers()
                    ->where('user_id', (int) $transaction->user_id)
                    ->first();
                if ($planUser && (int) $planUser->is_active !== 1) {
                    $planUser->update([
                        'is_active' => 1,
                        'user_term' => 'active',
                        'exist_customer' => '1',
            ...
            if (str_starts_with($planId, 'sub-')) {
                ...
                $planUser = $planUser ?? \App\Models\AdminUser::query()
                    ->customers()
                    ->where('user_id', (int) $transaction->user_id)
                    ->first();
                if ($planUser) {
                    $stripeSubscriptionId = trim((string) ($session['subscription'] ?? ''));
            ...
            $status = 'verified';
        } elseif ($transaction && in_array($type, ['checkout.session.completed', 'checkout.session.async_payment_succeeded'], true) && $paymentStatus === 'paid') {
            $result = $this->reconcileTransaction(
                $transaction,
                $site,
                StripeHostedCheckout::confirmedAmount($session),
            ...
            $result = $this->handleStripeInvoicePaid($event);
            $status = $result['ok'] ? 'verified' : 'failed';
        } elseif ($type === 'invoice.payment_failed') {
            $result = $this->handleStripeInvoiceFailed($event);
            $status = $result['ok'] ? 'marked_past_due' : 'failed';
        }
    ...
    private function handleStripeInvoicePaid(array $event): array
    {
        $invoice = (array) ($event['data']['object'] ?? []);
        $subscriptionId = trim((string) ($invoice['subscription'] ?? ''));

        if ($subscriptionId === '') {
            return ['ok' => false, 'message' => 'No subscription ID in invoice.'];
        }

        $customer = \App\Models\AdminUser::query()
            ->customers()
        ...
        if ($credits > 0) {
            \App\Support\CustomerBalance::incrementDeposit((int) $customer->user_id, (float) $credits);
        }
        ...
        return ['ok' => true, 'message' => 'Added '.number_format($credits, 2).' credits for subscription renewal.'];
    }

    private function handleStripeInvoiceFailed(array $event): array
    {
        $invoice = (array) ($event['data']['object'] ?? []);
        $subscriptionId = trim((string) ($invoice['subscription'] ?? ''));

        if ($subscriptionId === '') {
            return ['ok' => false, 'message' => 'No subscription ID in failed invoice.'];
        }
        ...
        return ['ok' => true, 'message' => 'Marked subscription as past_due for customer #'.$customer->user_id];
    }

    private function subscriptionPlanCredits(?string $plan): float
    {
        return match (strtolower(trim((string) $plan))) {
            'growth'     => 100,
            'studio'     => 200,
            'production' => 400,
            'enterprise' => 1000,
