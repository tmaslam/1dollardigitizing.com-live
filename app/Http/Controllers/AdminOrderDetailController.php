...
class AdminOrderDetailController extends Controller
{
    ...
    public function downloadAttachment(Attachment $attachment)
    {
        ...
        if (! is_file($path)) {
            SecurityAudit::record(null, 'files.attachment_missing', 'Admin attachment download failed because the file was not found on disk.', [
    ...
    public function previewAttachment(Request $request, Attachment $attachment)
    {
        ...
        if (! is_file($path)) {
            SecurityAudit::record(null, 'files.attachment_preview_missing', 'Admin attachment preview failed because the file was not found on disk.', [
                'attachment_id' => (int) $attachment->id,
                'order_id' => (int) $attachment->order_id,
                'file_source' => (string) $attachment->file_source,
                'resolved_path' => $path,
    ...
    public function complete(Request $request)
    {
        ...
        if (($page !== 'vector') && ! preg_match('/^\d+(\.\d+)?(\/\d+(\.\d+)?)*$/', $stitches)) {
            return $this->redirectToDetail($order->order_id, $page, $validated['back'] ?? null)
                ->withErrors(['stitches' => 'No. Of Stitches must be a numeric value. Use / to separate multiple files (e.g. 12000/8000).']);
        }
        ...
        if ($status === 'done' && is_numeric($adminAmount) && (float) $adminAmount <= 0) {
            return $this->redirectToDetail($order->order_id, $page, $validated['back'] ?? null)
    ...
    private function attachmentAbsolutePath(Attachment $attachment): string
    {
        ...
        $fallbackFolders = match (true) {
            ...
            $source === 'orderTeamImages' => ['team', 'quotes'],
            $source === 'team' => ['sewout'],
            default => [],
        };
    ...
    {
        $isRouted = (int) $order->assign_to > 0
            || (! is_null($order->assigned_group) && (string) $order->assigned_group !== '');

        if ($page === 'quote') {
            return match ($order->status) {
                'Underprocess' => ($isRouted ? 'assigned-quotes' : 'new-quotes'),
        ...
        return match ($order->status) {
            'Underprocess' => ($isRouted ? 'designer-orders' : 'new-orders'),
