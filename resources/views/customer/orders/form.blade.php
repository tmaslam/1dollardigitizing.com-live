...
@php
    ...
    $isQuote = ($flow['flow_context'] ?? 'order') === 'code';
...
@section('hero_text', $isCreate
    ? 'Complete the '.($isQuote ? 'quote' : 'order').' details below and upload your files.'
    : 'Update your '.($isQuote ? 'quote' : 'order').' details and files below.')
...
@section('content')
    ...
    <section class="content-card">
        <div class="section-head">
            <div>
                <h3>{{ $isCreate ? ($isQuote ? 'Quote Details' : 'Order Details') : 'Edit Order' }}</h3>
        ...
        <form method="post" action="{{ $formAction }}" enctype="multipart/form-data" class="form-grid">
            ...
            <label style="grid-column: 1 / -1;">
                ...
                <textarea name="comments" rows="4">{{ $commentsValue }}</textarea>
            </label>

            <label style="grid-column: 1 / -1;">
                <span>Upload Files <span class="required-mark">*</span></span>
                <input
                    type="file"
                    name="source_files[]"
                ...
                <input
                    ...
                    data-extra-upload
                >
                <button type="button" class="button secondary" style="margin-top:10px;font-size:0.82rem;" data-add-more-files>
                    + Upload 2nd File (Optional)
                </button>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.querySelectorAll('[data-add-more-files]').forEach((btn) => {
                btn.addEventListener('click', () => {
                    ...
                    const hiddenInputs = label?.querySelectorAll('[data-extra-upload][hidden]');
                    if (hiddenInputs && hiddenInputs.length > 0) {
                        hiddenInputs[0].hidden = false;
                        if (hiddenInputs.length === 1) {
                            btn.hidden = true;
                        } else {
                            btn.textContent = '+ Upload 3rd File (Optional)';
                        }
                    }
                });
            });
