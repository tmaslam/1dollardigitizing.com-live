...
@section('content')
    ...
    <section class="content-card">
        ...
        @if ($downloadsLocked)
            <div class="alert alert-error" style="margin-bottom:18px;">
                File downloads and previews are locked because you have outstanding unpaid billing. Please <a class="inline-link" href="{{ url('/view-billing.php') }}">clear your billing balance</a> to access all files. Files from paid orders remain fully accessible.
            </div>
        @endif
                        ...
                        @foreach ($sourceAttachments as $attachment)
                            ...
                            <li class="file-item">
                                ...
                                <div class="file-actions">
                                    @if ($downloadsLocked)
                                        <span class="status warning file-notice">Locked — clear billing to download</span>
                                    @else
    ...
    @if ($showApproveAction)
        @php
            $orderCost        = (float) preg_replace('/[^0-9.\-]/', '', (string) $order->total_amount);
            $isFreeOrder      = trim(strtolower((string) $order->total_amount)) === 'first order is free' || $orderCost <= 0;
            $hasSufficientBal = $isFreeOrder || ($availableBalance ?? 0) >= $orderCost - 0.001;
            $approveLabel     = $isFreeOrder ? 'Approve Order' : ($hasSufficientBal ? 'Send to Paid' : 'Approve & Add to Billing');
        @endphp
        <section class="content-card single-column">
            <div class="section-head">
                <div>
                    <h3>Customer Approval</h3>
                    <p>Completed orders still need your approval before they move to paid orders or billing.</p>
                </div>
            </div>

            @if (! $isFreeOrder)
                <div style="font-size:0.84rem; color:#526071; margin-bottom:14px;">
                    Order cost: <strong>{{ number_format($orderCost, 2) }} credits</strong>
                    &nbsp;·&nbsp;
                    Your balance: <strong style="color:{{ $hasSufficientBal ? '#3c9e6a' : '#e07b20' }}">{{ number_format($availableBalance ?? 0, 2) }} credits</strong>
                </div>
            @endif
            ...
            @if (! $isFreeOrder && ! $hasSufficientBal)
                <p style="margin-top:10px; font-size:0.83rem; color:#e07b20;">
                    You don't have enough credits — this order will be added to your billing.
                    File downloads will be locked until your billing is cleared.
                    <a class="inline-link" href="{{ url('/dashboard.php') }}">Top up your balance →</a>
                </p>
            @endif
