...
@section('title', ($mode === 'quote' ? 'Quotation' : 'Order').' Detail #'.($order->order_num ?: $order->order_id).' | 1Dollar Team Portal')
@section('page_heading', ($mode === 'quote' ? 'Quotation' : 'Order').' Detail #'.($order->order_num ?: $order->order_id))
...
@section('content')
    ...
    <section class="card">
        <div class="card-body">
            <h3 style="margin:0 0 6px;font-size:1.15rem;">Customer Comments</h3>
            <p class="muted" style="margin:0 0 18px;">Original comments and edit requests from the customer, stacked chronologically.</p>
            ...
            <div class="table-wrap">
                <table>
                    ...
                    @if (collect($customerComments)->isEmpty())
                        <tr><td colspan="2" class="muted">No customer comments available.</td></tr>
                    @else
                    @foreach ($customerComments as $comment)
                        <tr>
                            <td>{{ $comment->comments }}</td>
                            <td style="white-space:nowrap;font-size:0.82rem;color:#526071;">{{ $comment->date_added ?: '-' }}</td>
                        </tr>
                    @endforeach
                    @endif
                    </tbody>
    ...
    @if (($isFreelancePool ?? false) && ($teamUser->isFreelance()))
    {{-- Freelance quote panel: shown to freelancers viewing an open freelance pool job --}}
    <section class="card">
        <div class="card-body">
            <h3 style="margin:0 0 6px;font-size:1.15rem;">Submit Price Quote</h3>
            <p class="muted" style="margin:0 0 18px;">Review the job details above and submit your price quote. Admin will review all quotes and accept one.</p>

            @if (session('quote_success'))
                <div class="alert success" style="margin-bottom:16px;">{{ session('quote_success') }}</div>
            @endif
            @if ($errors->any())
                <div class="alert" style="margin-bottom:16px;">{{ $errors->first() }}</div>
            @endif

            @if (($myFreelanceQuote ?? null)?->status === 'accepted')
                <div class="alert success">Your quote of <strong>PKR {{ number_format((float) $myFreelanceQuote->quoted_price, 2) }}</strong> was accepted. This job is now assigned to you.</div>
            @elseif (($myFreelanceQuote ?? null)?->status === 'rejected')
                <div class="alert">Your quote of <strong>PKR {{ number_format((float) $myFreelanceQuote->quoted_price, 2) }}</strong> was not accepted on this order.</div>
            @else
            <form method="post" action="{{ url('/team/orders/'.$order->order_id.'/submit-quote') }}" class="toolbar">
                @csrf
                <input type="hidden" name="queue" value="{{ $queueKey }}">

                <div class="field">
                    <label for="quoted_price">Your Price (PKR)</label>
                    <input
                        id="quoted_price"
                        type="number"
                        name="quoted_price"
                        min="0.01"
                        step="0.01"
                        value="{{ old('quoted_price', $myFreelanceQuote?->quoted_price) }}"
                ...
                <div class="field" style="min-width:280px;">
                    <label for="quote_notes">Notes (optional)</label>
                    <textarea id="quote_notes" name="notes" rows="3" placeholder="Any questions or comments about this job?">{{ old('notes', $myFreelanceQuote?->notes) }}</textarea>
                </div>

                <div class="field" style="min-width:auto;">
                    <label>&nbsp;</label>
                    <button type="submit">{{ $myFreelanceQuote ? 'Update Quote' : 'Submit Quote' }}</button>
                </div>
            ...
            @if ($myFreelanceQuote)
                <p class="muted" style="margin-top:12px;">
                    Your current quote: <strong>${{ number_format((float) $myFreelanceQuote->quoted_price, 2) }}</strong>
                    &mdash; <span style="color:#b26a2a;font-weight:600;">Pending review</span>
                    &mdash; submitted {{ $myFreelanceQuote->created_at?->format('M d, Y g:i a') }}.
                    You can update your price or notes until admin accepts a quote.
                </p>
            @endif
            @endif
        </div>
    </section>
    @elseif (($isFreelancePool ?? false) && ($teamUser->is_supervisor ?? false) && $freelanceQuotes && $freelanceQuotes->isNotEmpty())
    {{-- Freelance quotes panel: shown to supervisors viewing an open freelance pool job --}}
    <section class="card">
        <div class="card-body">
            <h3 style="margin:0 0 6px;font-size:1.15rem;">Freelance Quotes</h3>
            <p class="muted" style="margin:0 0 18px;">Review submitted price quotes and accept one to assign the job.</p>
                            ...
                            <td>
                                @if ($quote->status === 'pending')
                                    <form method="post" action="{{ url('/team/supervisor/orders/'.$order->order_id.'/accept-quote') }}" style="display:inline;">
                                        ...
                                        <button type="submit" style="background:linear-gradient(135deg,#1e6a57,#114439);">Accept</button>
                                    </form>
                                @else
    ...
    @elseif (($isFreelancePool ?? false) && ($teamUser->is_supervisor ?? false))
    <section class="card">
        <div class="card-body">
            <h3 style="margin:0 0 6px;font-size:1.15rem;">Freelance Quotes</h3>
