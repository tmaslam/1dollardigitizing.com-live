...
@section('page_subheading', 'Manage team and supervisor accounts. Use the Status filter to narrow by active or inactive members.')
            ...
            <form method="get" action="{{ url('/v/show-all-teams.php') }}" class="toolbar">
                ...
                <div class="field">
                    ...
                    <select id="status" name="status">
                        ...
                        <option value="locked" @selected(request('status') === 'locked')>Inactive</option>
                    </select>
                </div>
        ...
        <div class="card-body">
            <div style="display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap;">
                <div>
                    ...
                    <p class="muted" style="margin:0;">Showing {{ $teams->total() }} team portal user{{ $teams->total() === 1 ? '' : 's' }}{{ request('status') === 'active' ? ' (active only)' : (request('status') === 'locked' ? ' (inactive only)' : '') }}.</p>
                </div>
                ...
                <table>
                    ...
                    @foreach ($teams as $team)
                        <tr>
                            <td class="action-col">
                                <div class="action-row">
                                    ...
                                    @if ((int) $team->is_active === 1)
                                        ...
                                        <form method="post" action="{{ url('/v/teams/'.$team->user_id.'/disable') }}" onsubmit="return confirm('Deactivate this team account?');">
                                            @csrf
                                            @foreach (request()->query() as $key => $value)
                                                <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                                            @endforeach
                                            <button type="submit" style="background:linear-gradient(135deg,#a24d2a,#7f2e14);">Deactivate</button>
                                        </form>
                                    @else
                                        <form method="post" action="{{ url('/v/teams/'.$team->user_id.'/unlock') }}" onsubmit="return confirm('Activate this team account and restore access?');">
                                            @csrf
                                            @foreach (request()->query() as $key => $value)
                                                <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                                            @endforeach
                                            <button type="submit" style="background:linear-gradient(135deg,#1f7a53,#145c3c);">Activate</button>
                                        </form>
                                        <form method="post" action="{{ url('/v/teams/'.$team->user_id.'/destroy') }}" onsubmit="return confirm('WARNING: This will PERMANENTLY delete this team member. This action cannot be undone. Continue?');">
                                            @csrf
                                            @foreach (request()->query() as $key => $value)
                                                <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                                            @endforeach
                                            <button type="submit" style="background:linear-gradient(135deg,#b91c1c,#991b1b);">Delete</button>
                                        </form>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td>
                                @if ($team->is_supervisor)
                                    <span class="badge" style="background:rgba(13,110,163,0.12);color:#0d6ea3;border-color:rgba(13,110,163,0.24);">Supervisor</span>
                                @elseif ($team->isFreelance())
                                    <span class="badge" style="background:rgba(197,107,34,0.12);color:#9d4e1f;border-color:rgba(197,107,34,0.24);">Freelance Team</span>
                                @else
                                    <span class="badge" style="background:rgba(34,139,94,0.12);color:#1f7a53;border-color:rgba(34,139,94,0.24);">In-house Team</span>
                                @endif
                            </td>
