...
@section('content')
    ...
    <section class="card">
        <div class="card-body">
            ...
            <style>
                .detail-hero-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 16px; }
                .detail-hero-grid .hero-card {
                    padding: 20px 22px;
                    border-radius: 22px;
                    background: #fff;
                    border: 1px solid rgba(24, 34, 45, 0.12);
                }
                .detail-hero-grid .hero-card .label {
                    display: block;
                    font-size: 0.78rem;
                    font-weight: 800;
                ...
                .detail-hero-grid .hero-card .value {
                    display: block;
                    margin-top: 10px;
                    font-size: 1.35rem;
                    font-weight: 700;
                ...
                .detail-hero-grid .hero-card .sublabel {
                    display: block;
                    margin-top: 6px;
                    font-size: 0.9rem;
                    font-weight: 600;
            ...
            <div class="detail-hero-grid">
                ...
                <div class="hero-card">
                    ...
                    <span class="value">{{ $order->work_type_label ?: '-' }}</span>
                    <span class="sublabel">{{ $order->flow_context_label }} / {{ $order->work_type_label }}</span>
                </div>
                <div class="hero-card">
                ...
                <div class="hero-card">
                    ...
                    @elseif ($order->assigned_group === 'freelance')
                        <span class="value">Freelance Pool</span>
                    @elseif ($order->assigned_group === 'inhouse')
                        <span class="value">In-House Pool</span>
                    @else
                ...
                <div class="hero-card">
                    ...
                    <span class="value">{{ $offerAdjustedAmount ?? ($order->total_amount ?: '0.00') }}</span>
                </div>
                <div class="hero-card">
                    <span class="label">Payment</span>
                    <span class="value">{{ $customerPaidFlag ? 'Paid' : 'Unpaid' }}</span>
                </div>
                <div class="hero-card">
                    <span class="label">Approval</span>
                    <span class="value">{{ ($approvedBillingFlag || $order->status === 'approved') ? 'Approved' : 'Pending' }}</span>
                </div>
            </div>
            ...
            <details class="card subcard" style="border-radius:22px;overflow:hidden;">
                <summary style="list-style:none;cursor:pointer;padding:18px 22px;display:flex;justify-content:space-between;align-items:center;gap:12px;">
                    <h4 style="margin:0;font-size:1.05rem;">Customer Comments</h4>
                    <span class="badge" style="font-size:0.75rem;">No Comments</span>
                </summary>
                <div style="padding:0 22px 22px;">
                    <div class="table-wrap">
                        <table>
                            ...
                            <tbody>
                                <tr><td class="muted">No comments in this section.</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
    ...
    <section class="card">
        <div class="card-body">
            @foreach (['order' => 'Customer Source Files', 'complaint' => 'Customer Complaint Attachments', 'team' => 'Team / Sewout Attachments'] as $key => $label)
                @php
                    $complaintEmpty = $key === 'complaint' && collect($attachmentGroups[$key] ?? [])->isEmpty();
                @endphp
                @if ($complaintEmpty)
                <details class="card subcard" style="border-radius:22px;overflow:hidden;">
                    <summary style="list-style:none;cursor:pointer;padding:18px 22px;display:flex;justify-content:space-between;align-items:center;gap:12px;">
                        <h4 style="margin:0;font-size:1.05rem;">{{ $label }}</h4>
                        <span class="badge" style="font-size:0.75rem;">Empty</span>
                    </summary>
                    <div style="padding:0 22px 22px;">
                @else
                <div class="card subcard">
    ...
    <section class="card">
        <div class="card-body">
            ...
            @foreach ($commentSections as $section)
                ...
                <details class="card subcard" style="border-radius:22px;overflow:hidden;">
                    <summary style="list-style:none;cursor:pointer;padding:18px 22px;display:flex;justify-content:space-between;align-items:center;gap:12px;">
                        <h4 style="margin:0;font-size:1.05rem;">{{ $section['label'] }}</h4>
                        <span class="badge" style="font-size:0.75rem;">No Comments</span>
                    </summary>
                    <div style="padding:0 22px 22px;">
                        <div class="table-wrap">
    ...
    <section class="card">
        <div class="card-body" style="padding:0;">
            <details style="border-radius:22px;overflow:hidden;">
                <summary style="list-style:none;cursor:pointer;padding:18px 22px;display:flex;justify-content:space-between;align-items:center;gap:12px;">
                    <div>
                        <h4 style="margin:0;font-size:1.05rem;">Customer Delivery Access</h4>
                        <p class="section-copy" style="margin:4px 0 0;">{{ $customerDeliveryGate['reason'] }}</p>
                    </div>
                    <span class="badge">Expand</span>
                </summary>
                <div style="padding:0 22px 22px;">
                    @if ((float) $customerDeliveryGate['prepaid_amount'] > 0 || (float) $customerDeliveryGate['outstanding_due'] > 0)
                        <p class="muted" style="margin:0 0 14px;">Credit balance: {{ number_format((float) $customerDeliveryGate['prepaid_amount'], 2) }} | Outstanding billing: {{ number_format((float) $customerDeliveryGate['outstanding_due'], 2) }}</p>
                    @endif
