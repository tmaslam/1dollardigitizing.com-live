...
@include('shared.file-preview-styles')
        ...
        * { box-sizing: border-box; }
        ...
        .brand--active {
            ...
            box-shadow: inset 0 1px 0 rgba(255,255,255,0.16), 0 0 0 1px rgba(255,255,255,0.12);
        }
        .brand-logo {
            display: block;
            width: auto;
        ...
        .section-dropdown { margin-top: 18px; }
        .dropdown-toggle {
            width: 100%;
            display: flex;
            align-items: center;
        ...
        .dropdown-toggle:hover { background: rgba(255,255,255,0.1); color: rgba(255,255,255,0.95); }
        ...
        .dropdown-toggle[aria-expanded="true"] .chevron { transform: rotate(-135deg); margin-top: 4px; }
        .dropdown-panel {
            display: grid;
            gap: 10px;
            overflow: hidden;
        ...
        tr.row-digitizing td { box-shadow: inset 0 -3px 0 rgba(34, 139, 94, 0.18); }
        tr.row-vector td { box-shadow: inset 0 -4px 0 rgba(13, 71, 161, 0.55); }
        th {
            font-size: 0.78rem;
            text-transform: uppercase;
...
@php
    ...
    $activeFreelancePayments = request()->is('v/freelance-payments.php');
...
<div class="shell">
    <aside class="sidebar">
        ...
        <a href="{{ url('/v/welcome.php') }}" class="brand {{ request()->is('v/welcome.php') ? 'brand--active' : '' }}">
            <img class="brand-logo" src="{{ url('/images/logo.png') }}" alt="1 Dollar Digitizing">
            <p class="brand-label">Admin</p>
        </a>
        ...
        <div class="nav-list">
            <div class="nav-card">
                @foreach ($orderQueueNav as $queueItem)
                    <a href="{{ $queueItem['url'] }}" class="{{ (request()->is('v/orders/*') || request()->is('v/orders.php')) && $activeQueue !== null && $activeQueue === $queueItem['key'] ? 'active' : '' }}"><span>{{ $queueItem['label'] }}</span><span class="count">{{ $queueItem['count'] }}</span></a>
                @endforeach
                <a href="{{ url('/v/payment-recieved.php') }}" class="{{ request()->is('v/payment-recieved.php') ? 'active' : '' }}"><span>Paid Orders</span><span class="count">{{ $navCounts['received_payments'] ?? 0 }}</span></a>
            </div>
        </div>
        ...
        <div class="nav-list">
            <div class="nav-card">
                ...
                <a href="{{ url('/v/freelance-payments.php') }}" class="{{ $activeFreelancePayments ? 'active' : '' }}"><span>Freelance Payments</span></a>
            </div>
        </div>
        ...
        <div class="section-dropdown">
            <button type="button" class="dropdown-toggle" data-dropdown-toggle="security-panel" aria-expanded="false">
                <span>Security</span><span class="chevron"></span>
            </button>
            <div class="nav-list dropdown-panel collapsed" id="security-panel">
                <div class="nav-card">
        ...
        <div class="nav-list">
            <div class="nav-card">
                <a href="{{ url('/v/blogs') }}" class="{{ request()->is('v/blogs') || request()->is('v/blogs/*') ? 'active' : '' }}">
        ...
        <div class="section-dropdown">
            <button type="button" class="dropdown-toggle" data-dropdown-toggle="extras-panel" aria-expanded="false">
                <span>Extras</span><span class="chevron"></span>
            </button>
            <div class="nav-list dropdown-panel collapsed" id="extras-panel">
                <div class="nav-card">
        ...
        <div class="section-dropdown">
            <button type="button" class="dropdown-toggle" data-dropdown-toggle="admin-panel" aria-expanded="false">
                <span>Admin</span><span class="chevron"></span>
            </button>
            <div class="nav-list dropdown-panel collapsed" id="admin-panel">
                <div class="nav-card">
                    <a href="{{ url('/v/change-password.php') }}" class="{{ request()->is('v/change-password.php') ? 'active' : '' }}"><span>Change Password</span></a>
                    <a href="{{ url('/v/site-contact.php') }}" class="{{ request()->is('v/site-contact.php') ? 'active' : '' }}"><span>Company Contact Info</span></a>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('[data-dropdown-toggle]').forEach(function (btn) {
        ...
        const hasActiveLink = panel.querySelector('a.active') !== null;
        if (hasActiveLink) {
...
@include('shared.file-preview-script')
    ...
    <style>
        ...
        .stt-r { display: block; font-size: 2.6rem; line-height: 1; transform: rotate(-45deg); }
        .stt-f {
            display: block;
            width: 8px;
            height: 14px;
